...
def save_persona_vector(model_name, pos_path, neg_path, trait, save_dir):
    ...
    persona_pos_effective, persona_neg_effective, persona_pos_effective_prompts, persona_neg_effective_prompts, persona_pos_effective_responses, persona_neg_effective_responses = get_persona_effective(pos_path, neg_path)
    ...
    persona_effective_prompt_avg_diff = torch.nn.functional.normalize(persona_effective_prompt_avg_diff, p=2, dim=1)
    ...
    persona_effective_response_avg_diff = torch.nn.functional.normalize(persona_effective_response_avg_diff, p=2, dim=1)
    ...
    persona_effective_prompt_last_diff = torch.nn.functional.normalize(persona_effective_prompt_last_diff, p=2, dim=1)
...
if __name__ == "__main__":
    ...
    save_persona_vector(args.model_name, args.pos_path, args.neg_path, args.trait, args.save_dir)